...
module NbaPlayers
  class CLI
    URL = 'https://www.washingtonpost.com/graphics/2017/sports/nba-top-100-players-2017/?noredirect=on&utm_term=.adcc13ae7e38'.freeze
    COLORIER = lambda { |player|
      color = ''
    ...
    def self.start
      ...
      until input == 'n'
        opt = -1
        until opt.positive? and opt <= 10
          puts "Invalid input!!! \n" if opt.zero? or opt >= 10
          rows = [
        ...
        if opt != 9
          ...
          until input == 'n'
            ...
            n_trend = 1 if n_trend.zero?
            n_trend = 3 if n_trend > 3

            choices = %w[neutral up down]
    ...
    def self.print_players(from_number, order = 'rank', stats = '')
      rows = if stats == ''
               NbaPlayers::Players.all[from_number - 1, 20].each.with_index(from_number).map do |player, index|
                 [index, player.name.colorize(COLORIER.call(player)), "#{order}:#{player.send(order)}"]
               end
             else
               NbaPlayers::Players.all[from_number - 1, 20].each.with_index(from_number).map do |player, index|
                 [index, player.name.colorize(COLORIER.call(player)), "#{stats} : #{player.send(order)[stats.to_sym]}"]
               end
             end
    ...
    def self.print_player(player)
      table = Terminal::Table.new title: player.name.to_s, rows: [
        ['Age', player.statistics[:AGE].colorize(COLORIER.call(player))],
        ['Position', player.position.colorize(COLORIER.call(player))],
        ['Team', player.team.colorize(COLORIER.call(player))],
        ['Rank', player.rank.to_s.colorize(COLORIER.call(player))],
        ['PPG', player.statistics[:PPG].to_s.colorize(COLORIER.call(player))],
        ['RPG', player.statistics[:RPG].to_s.colorize(COLORIER.call(player))],
        ['APG', player.statistics[:APG].to_s.colorize(COLORIER.call(player))],
        ['BLK', player.statistics[:BLK].to_s.colorize(COLORIER.call(player))],
        ['FT', player.statistics[:FT].to_s.colorize(COLORIER.call(player))],
        ['Steal', player.statistics[:STL].to_s.colorize(COLORIER.call(player))],
        ['FG', player.statistics[:FG].to_s.colorize(COLORIER.call(player))],
        ['3PT', player.statistics[:THREEPT].to_s.colorize(COLORIER.call(player))]
    ...
    def self.print_players_group(by_input)
      ...
      NbaPlayers::Players.group(by_input).each.with_index(1) do |player, i|
        arr.push i, player.name.colorize(COLORIER.call(player)), ' '
        if arr.size == 6
          rows << arr
          arr = []
        end
